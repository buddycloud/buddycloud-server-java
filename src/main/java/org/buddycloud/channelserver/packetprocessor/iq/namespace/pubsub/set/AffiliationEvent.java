...
public class AffiliationEvent extends PubSubElementProcessorAbstract {
	...
	private void saveUpdatedAffiliation() throws NodeStoreException {
		...
		Affiliations affiliation = Affiliations.valueOf(requestedAffiliation.attributeValue("affiliation"));
	...
	private boolean subscriberHasCurrentAffiliation() throws NodeStoreException {
		currentAffiliation = channelManager.getUserAffiliation(node, new JID(requestedAffiliation.attributeValue("jid")));

		if (currentAffiliation.getAffiliation().equals(Affiliations.none)) {
			setErrorCondition(PacketError.Type.modify,
	...
	private boolean actorHasPermissionToAuthorize() throws NodeStoreException {
		...
		return affiliation.getAffiliation().in(Affiliations.moderator, Affiliations.owner);
	}
