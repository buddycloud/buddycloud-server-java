...
public class SubscriptionEvent extends PubSubElementProcessorAbstract
{
	...
	public void process(Element elm, JID actorJID, IQ reqIQ, Element rsm)
        ...
        try {
			if ((false == checkNode())
			    || (false == userHasPermissionToAuthorize())
					) {
	...
	private boolean userHasPermissionToAuthorize() throws DataStoreException {
		NodeSubscriptionImpl subscription = dataStore.getUserSubscriptionOfNode(actor.toBareJID(), node);
		if (null == subscription) {
			setErrorCondition(PacketError.Type.auth, PacketError.Condition.not_authorized);
			return false;
		}
		if ((false ==subscription.getAffiliation().equals(Affiliations.moderator))
		    && (false == subscription.getAffiliation().equals(Affiliations.owner))
		) {
			setErrorCondition(PacketError.Type.auth,  PacketError.Condition.not_authorized);
		    return false;
		}
	...
	private boolean checkNode() throws DataStoreException {
		if (null == node) {
