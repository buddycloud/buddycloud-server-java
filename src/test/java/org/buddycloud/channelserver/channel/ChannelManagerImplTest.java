...
public class ChannelManagerImplTest {
  ...
  public void testCreatePersonalChannel() throws Exception {
    ...
    verify(nodeStore).createNode(channelJID, Conf.getPostChannelNodename(channelJID),
        Conf.getDefaultPostChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getStatusChannelNodename(channelJID),
        Conf.getDefaultStatusChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getGeoPreviousChannelNodename(channelJID),
        Conf.getDefaultGeoPreviousChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getGeoCurrentChannelNodename(channelJID),
        Conf.getDefaultGeoCurrentChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getGeoNextChannelNodename(channelJID),
        Conf.getDefaultGeoNextChannelConf(channelJID));
  }
  ...
  public void testCreatePersonalChannelSomeNodesExist() throws Exception {
    ...
    verify(nodeStore, never()).createNode(channelJID, Conf.getPostChannelNodename(channelJID),
        Conf.getDefaultPostChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getStatusChannelNodename(channelJID),
        Conf.getDefaultStatusChannelConf(channelJID));
    verify(nodeStore).createNode(channelJID, Conf.getGeoPreviousChannelNodename(channelJID),
  ...
  public void returnsEphemeralNodeIsFalseIfNoConfiguratonVaueProvided() throws Exception {
    Mockito.when(nodeStore.getNodeConfValue(Mockito.anyString(), Mockito.eq(Ephemeral.FIELD_NAME)))
        .thenReturn(null);
  ...
  public void returnsEphemeralNodeIsFalseIfConfigurationValueIsNotTrue() throws Exception {
    Mockito.when(nodeStore.getNodeConfValue(Mockito.anyString(), Mockito.eq(Ephemeral.FIELD_NAME)))
        .thenReturn("no way is this true");
    assertFalse(channelManager.isEphemeralNode(user1));
  }
