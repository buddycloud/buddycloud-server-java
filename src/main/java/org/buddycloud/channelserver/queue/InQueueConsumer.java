...
public class InQueueConsumer extends QueueConsumer {
    ...
    protected void consume(Packet p) {
        try {
            ...
            if (p instanceof IQ) {
            	new IQProcessor(outQueue, conf, channelManagerFactory.create()).process((IQ) p);
            } else if (p instanceof Message) {
            	new MessageProcessor(outQueue, inQueue, conf, channelManagerFactory.create()).process((Message) p);
            } else {
