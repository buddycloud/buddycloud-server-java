...
public class ConfigurationTest extends IQTestHandler {
    ...
    public void setUp() throws Exception {
        ...
        readStanzaAsIq("/iq/pubsub/subscriptions/reply-with-node.stanza");
        readStanzaAsIq("/iq/pubsub/subscriptions/reply-no-node.stanza");
    ...
    public void testEmptyNodeValueCausesNoAction() throws Exception {
        IQ result =
                toIq("<iq type=\"result\" id=\"subscriptions1\" " + "from=\"channels.shakespeare.lit\" " + "to=\"channels.denmark.lit\">"
                        + "<pubsub xmlns=\"http://jabber.org/protocol/pubsub#owner\">" + "<configure node=\"\" />" + "</pubsub>" + "</iq>");

        confResult.process(element, jid, result, null);

        Mockito.verify(channelManager, Mockito.times(0)).nodeExists(Mockito.anyString());
    }
    ...
    public void testIfNodeIsUnknownThenItIsAddedToDatabase() throws Exception {
        IQ result =
                toIq("<iq type=\"result\" id=\"subscriptions1\" " + "from=\"channels.shakespeare.lit\" " + "to=\"channels.denmark.lit\">"
                        + "<pubsub xmlns=\"http://jabber.org/protocol/pubsub#owner\">" + "<configure node=\"some-node\" />" + "</pubsub>" + "</iq>");

        confResult.process(element, jid, result, null);

        Mockito.when(channelManager.nodeExists(Mockito.anyString())).thenReturn(false);
        Mockito.verify(channelManager, Mockito.times(1)).nodeExists(Mockito.anyString());
