...
public class DiscoResult implements PacketProcessor<IQ> {
    ...
    public void process(IQ reqIQ) throws Exception {
        requestIq = reqIQ;
        node = requestIq.getElement().element("query").attributeValue("node");
        if ((null == node) && federatedQueueManager.isFederatedDiscoInfoRequest(reqIQ.getID())) {
            federatedRequest();
            return;
        }
        try {
    ...
    private void federatedRequest() throws ComponentException {
        ...
        federatedQueueManager.processDiscoInfoResponse(requestIq.getFrom(), requestIq.getID(), identities);
    }
