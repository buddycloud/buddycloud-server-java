...
public class AffiliationEventTest extends IQTestHandler {
  ...
  public void userWhoIsntOwnerOrModeratorCantUpdateAffiliation() throws Exception {
    ...
    Mockito.when(channelManager.getNodeMembership(Mockito.anyString(), Mockito.eq(jid)))
        .thenReturn(
            new NodeMembershipImpl(node, jid, Subscriptions.subscribed, Affiliations.publisher,
                null));
  ...
  public void passingInvalidAffiliationTypeSetsAffiliationToNone() throws Exception {
    ...
    IQ request =
        toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst(
            "affiliation='member'", "affiliation='i-can-haz-all-the-items'"));
  ...
  public void testPassingValidAffiliationTypeUpdatesAffiliation() throws Exception {
    ...
    IQ request =
        toIq(readStanzaAsString("/iq/pubsub/affiliation/affiliationChange.stanza").replaceFirst(
            "affiliation='member'", "affiliation='moderator'"));
  ...
  public void testPassingValidAffiliationSendsOutExpectedNotifications() throws Exception {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (packet.getElement().getName().equals("iq")
          && packet.getTo().equals(new JID("francisco@denmark.lit/barracks"))) {
        hasIqResult = true;
      }
      ...
      if (packet.getElement().getName().equals("message")
        ...
        Assert.assertEquals(node, packet.getElement().element("event").element("affiliations")
            .attributeValue("node"));
        Assert.assertTrue(packet.toXML().contains(JabberPubsub.NS_PUBSUB_EVENT));
        Assert.assertEquals(Affiliations.moderator.toString(), packet.getElement().element("event")
            .element("affiliations").element("affiliation").attributeValue("affiliation"));
  ...
  public void canNotUpdateOwnAffiliation() throws Exception {
    ...
    Assert.assertEquals(event.CAN_NOT_MODIFY_OWN_AFFILIATION, error.getApplicationConditionName());
  ...
  @Test
  public void ephemeralNodesCanOnlyHaveOwnersAndModerators() throws Exception {
