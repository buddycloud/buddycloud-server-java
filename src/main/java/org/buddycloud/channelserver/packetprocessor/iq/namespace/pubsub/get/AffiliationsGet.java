...
public class AffiliationsGet extends PubSubElementProcessorAbstract {
    ...
    public AffiliationsGet(BlockingQueue<Packet> outQueue, ChannelManager channelManager) {
        ...
        this.acceptedElementName = XMLConstants.AFFILIATIONS_ELEM;
    }
    ...
    public void process(Element elm, JID actorJID, IQ reqIQ, Element rsm) throws Exception {
        ...
        node = request.getChildElement().element(this.acceptedElementName).attributeValue(XMLConstants.NODE_ATTR);
        ...
        boolean isProcessedLocally = (node == null) ? getUserMemberships(affiliations) : getNodeAffiliations(affiliations);
    ...
    private boolean getNodeAffiliations(Element affiliations) throws NodeStoreException, InterruptedException {
        ...
        for (NodeMembership nodeMembership : nodeMemberships) {
            if (!nodeMembership.getSubscription().equals(Subscriptions.subscribed) && !actor.toBareJID().equals(nodeMembership.getUser().toBareJID())
                    && !isOwnerModerator()) {
                continue;
            }
            ...
            affiliation.addAttribute(XMLConstants.AFFILIATION_ELEM, nodeMembership.getAffiliation().toString());
            affiliation.addAttribute(XMLConstants.JID_ATTR, nodeMembership.getUser().toString());
            if ((actor.toBareJID().equals(nodeMembership.getUser().toBareJID()) || isOwnerModerator()) && (null != nodeMembership.getInvitedBy())) {
                affiliation.addAttribute(XMLConstants.INVITED_BY_ATTR, nodeMembership.getInvitedBy().toBareJID());
            }
        }
    ...
    private boolean getUserMemberships(Element affiliations) throws NodeStoreException, InterruptedException {
        ...
        if (!channelManager.isCachedJID(request.getFrom()) && !Configuration.getInstance().isLocalJID(actor)) {
